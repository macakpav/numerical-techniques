...
%% Cross section
figure()
dif = abs(result.U.data(1,:) - casedef.S.Sol.data(1,:))/norm(casedef.S.Sol.data);
%dif = sum(dif,1);
...
figure()
dif = abs(result.U.data(2,:) - casedef.S.Sol.data(2,:))/norm(casedef.S.Sol.data);
%dif = sum(dif,1);
diffield = Field(casedef.dom.allCells, 0); % Velocity [m/s] (vector);
set(diffield, dif);
fvmplotfield(diffield, scale, lw);
